...
public:
    ...
    AggregateFunctionPtr getOwnNullAdapter(
        const AggregateFunctionPtr & nested_function, const DataTypes & types, const Array & params) const override
    {
        return std::make_shared<AggregateFunctionNullVariadic<false, false, true>>(nested_function, types, params);
    }
        ...
        switch (Base)
        {
            ...
            case LAST_MATCH:
                for (UInt64 i = 0; i < data.value.size(); ++i)
                {
                    ...
                    if (data.value[reversed_i]->events_bitset.test(0) && data.value[i]->can_be_base)
                    {
                        exist = true;
                        return reversed_i;
                    }
