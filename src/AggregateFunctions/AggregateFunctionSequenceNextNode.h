...
struct SequenceNextNodeNodeBase
{
...
struct SequenceNextNodeNodeString : public SequenceNextNodeNodeBase<SequenceNextNodeNodeString>
{
    using Node = SequenceNextNodeNodeString;
...
template <typename T, typename Node, bool descending_order>
...
template <typename T, typename Node, bool descending_order>
class SequenceNextNodeGeneralImpl final
    : public IAggregateFunctionDataHelper<SequenceNextNodeGeneralData<T, Node, descending_order>, SequenceNextNodeGeneralImpl<T, Node, descending_order>>
{
    using Data = SequenceNextNodeGeneralData<T, Node, descending_order>;
...
public:
    SequenceNextNodeGeneralImpl(const DataTypePtr & data_type_, const DataTypes & arguments, UInt64 max_elems_ = std::numeric_limits<UInt64>::max())
        : IAggregateFunctionDataHelper<SequenceNextNodeGeneralData<T, Node, descending_order>, SequenceNextNodeGeneralImpl<T, Node, descending_order>>(
            {data_type_}, {})
    ...
    void merge(AggregateDataPtr place, ConstAggregateDataPtr rhs, Arena * arena) const override
    {
        ...
        using Comparator = typename SequenceNextNodeGeneralData<T, Node, descending_order>::Comparator;
    ...
    UInt64 getNextNodeIndex(Data & data) const
    {
        ...
        if (events_size == 1)
        {
            ...
            {
                const auto & events_bitmap = data.value[i]->events_bitmap;

                if (!(events_bitmap & 1) && found_first)
                    return i;
                else if (events_bitmap & 1)
                    found_first = true;
            }
        }
        else
        {
            for (UInt64 i = 0; i < data.value.size() - events_size; ++i)
            {
                UInt64 j = 0;
                for (; j < events_size; ++j)
                    if ((data.value[i + j]->events_bitmap & (1 << j)) == 0)
                        break;
