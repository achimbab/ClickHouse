...
inline AggregateFunctionPtr createAggregateFunctionGroupArrayImpl(const DataTypePtr & argument_type, TArgs ... args)
{
    ...
    if (which.idx == TypeIndex::String)
        ...
        throw Exception("Unsupported type for aggregate function " + name + ", should be String";
...
{
    if (descending_order)
        return std::make_shared<SequenceNextNodeGeneralImpl<TYPE, SequenceNextNodeNodeString, true>>(data_type, argument_types);
    else
        return std::make_shared<SequenceNextNodeGeneralImpl<TYPE, SequenceNextNodeNodeString, false>>(data_type, argument_types);
}
...
AggregateFunctionPtr createAggregateFunctionSequenceNextNode(const std::string & name, const DataTypes & argument_types, const Array & parameters)
{
    ...
    if (parameters.size() == 1)
    {
        ...
        if (type != Field::Types::Int64 && type != Field::Types::UInt64)
               throw Exception("Parameter for aggregate function " + name + " should be 0 or 1", ErrorCodes::BAD_ARGUMENTS);
...
void registerAggregateFunctionSequenceNextNode(AggregateFunctionFactory & factory)
{
    AggregateFunctionProperties properties = { 
        // The sequenceNextNode returns a null as a default value.
