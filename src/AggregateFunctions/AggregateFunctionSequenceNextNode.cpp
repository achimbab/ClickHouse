...
{
    if (base == HEAD)
        return std::make_shared<SequenceNextNodeImpl<T, NodeString, Direction, HEAD>>(data_type, argument_types);
    else if (base == TAIL)
        return std::make_shared<SequenceNextNodeImpl<T, NodeString, Direction, TAIL>>(data_type, argument_types);
    else if (base == FIRST_MATCH)
        return std::make_shared<SequenceNextNodeImpl<T, NodeString, Direction, FIRST_MATCH>>(data_type, argument_types);
    else
        return std::make_shared<SequenceNextNodeImpl<T, NodeString, Direction, LAST_MATCH>>(data_type, argument_types);
}
